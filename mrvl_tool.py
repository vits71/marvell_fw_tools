...
def read_chunk():
    ...
    for chunk_type, chunk_addr, chunk_len in struct.iter_unpack(chunk_hdr_format, chunk_hdr):
        ...
        if chunk_type == 1:
            ...
            chunk_data_crc = \
                struct.unpack(chunk_data_crc_format, args.infile.read(struct.calcsize(chunk_data_crc_format)))[0]
            if args.verbose > 1:
                print('Read {} bytes of chunk data. Chunk CRC: {:#x}, calculated CRC: {:#x}'.format(len(chunk_data),
                                                                                                    chunk_data_crc,
                                                                                                    crc_data_calc))
...
parser_copy.add_argument('-c', '--count', type=int,
                         help='number of chunks to copy (excluding last chunk), no count means all chunks')
        ...
        try:
            ...
            for patch_address in args.address:
                ...
                if patch_address in chunk_range:
                    ...
                    if args.verbose > 2:
                        print('Patched data {}, at address {:#x}, len {} bytes'.format(patch_data, patch_address,
                                                                                       len(patch_data)))
...
parser_modify.add_argument('-a', '--address', type=lambda x: int(x, 0), action='append', required=True,
                           help='modification address start (can use 0x for hex)')
parser_modify.add_argument('-d', '--data', type=lambda x: ast.literal_eval(x), action='append', required=True,
